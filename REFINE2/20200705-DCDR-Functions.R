...
AIPW_Auto <- function(data, exposure, outcome, covarsT, covarsO, learners, control){
  ...
  pi_fit <- SuperLearner(Y=as.matrix(data[, exposure]), X=data[, covarsT], family=binomial(), 
                         SL.library=c(learners,rf_names), cvControl=control)
  mu_fit <- SuperLearner(Y=as.matrix(data[, outcome]), X=data[, c(exposure, covarsO)], family=binomial(), 
                         SL.library=c(learners,rf_names), cvControl=control)
...
# TMLE Estimator
TMLE <- function(data, exposure, outcome, covarsT, covarsO, learners, control,outForm, expForm){
  # Bound data
  ...
  # print(head(data.frame(model.matrix(object = as.formula(outForm), data=data))))
...
TMLE_Auto <- function(data, exposure, outcome, covarsT, covarsO, learners, control){
  # Bound data
  data <- data.frame(data)
  data <- data %>% mutate(YT=ifelse(YT<0.005, 0.005, ifelse(YT>0.995,0.995, YT)))
  ...
  pi_fit <- SuperLearner(Y=as.matrix(data[, exposure])[,1], X=exp_pred_mat,
                         family=binomial(), SL.library=c(learners, rf_names), cvControl=control) #CHANGE
  mu_fit <- SuperLearner(Y=as.matrix(data[, "YT"])[,1], X=out_pred_mat,
                         SL.library=c(learners, rf_names), cvControl=control)
