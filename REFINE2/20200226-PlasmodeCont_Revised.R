...
PlasmodeContNew <- function(formulaOut=NULL, objectOut=NULL,formulaExp=NULL,objectExp=NULL, data, idVar,
                              ...
                              outVar=NULL,generateA=T,sims.ver= "plas")
{
  require(glm2)
  require(mgcv)
  # library(gam)
  ...
  if(is.null(formulaOut)==FALSE & is.null(formulaExp)==FALSE & is.null(objectOut)==TRUE& is.null(objectExp)==TRUE)
  {
    ...
    # Change 2022-Feb-09: inject this one as ATE
    modOutforATE <- glm2(as.formula("Y~1+A"), family = "gaussian", x, control=glm.control(trace=F)) ## outcome model coefficients
    
    bnew <- c(coef(modOutCont)[1], MMOut*coef(modOutCont)[-1]) # find intercept value needed to get approximate mean outcome
    # Change 2021-Nov-25: we just use the regression coefficient as the simulation effect
    # bnew <- replace(bnew, names(coef(modOutCont)) == exposure, effectOR) # "inject" desired RD (beta coeff)
    bnew <- replace(bnew, names(coef(modOutCont)) == exposure, coef(modOutforATE)[2]) # "inject" desired RD (beta coeff)
    ...
    return(list(TrueOutBeta = bnew, TrueExpBeta = bnewExp, 
                ...
                RegEff = bnew[which(names(coef(modOutCont)) == exposure)]))
  }
