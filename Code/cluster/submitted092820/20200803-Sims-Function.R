...
draw_sims_hard <- function(i){
  Z1 <- runif(Nsamp, 0.5,2); 
  Z2 <- runif(Nsamp, -2,2); Z3 <- runif(Nsamp, -2,2); 
  Z4 <- runif(Nsamp, -2,2); Z5 <- runif(Nsamp, -2,2); 
  Pr_A <- expit(-Z1 + 0.5 * Z2 - Z3 -0.1*Z4 + Z5 +0.75 * Z5^2)
  A <- rbinom(length(Pr_A), 1, Pr_A)
  Y <- Effect_Size*A + 27.4 *  Z1 +  13.7 * (Z2+Z3+Z4)
  Y <- Y + rnorm(Nsamp, 0, 1)
  C1 <-  exp(Z1); C2 <- Z2  /(1+exp(Z1)) + 10;
  C3 <- (Z1*Z3/ 25  +  0.6)^3;
  C4 <- (Z2 + Z4+20)^2; C5 <- Z5
